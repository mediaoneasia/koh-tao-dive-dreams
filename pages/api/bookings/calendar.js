...
async function selectBookings({ userId } = {}) {
	...
	query = query.order('created_at', { ascending: false }).limit(1500);
...
export default async function handler(req, res) {
	...
	// Support both: public token-based and authenticated user calendar
	let userId = null;
	let isPublic = false;
	// Check for calendar token (public calendar)
	if (CALENDAR_TOKEN) {
	...
	// If not public, check for user auth
	if (!isPublic) {
	...
	try {
		const rows = await selectBookings({ userId: isPublic ? undefined : userId });
